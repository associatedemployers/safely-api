...
var Grant = require('../../models/grant'),
    CompanyUser = require('../../models/company-user');
...
module.exports = function ( options = {} ) {
  return function *(next) {
    ...
    if ( process.env.NODE_ENV === 'test' && this.request.get('X-Test-User') ) {
      this.user = yield CompanyUser.findById(this.request.get('X-Test-User')).populate('company');
      yield next;
      return;
    }
    ...
    if ( !grant || grant.isExpired ) {
      this.status = 401;
      this.body = 'Grant is either expired or non-existant';
      return;
    }
