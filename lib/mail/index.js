...
var path          = require('path'),
    ...
    helpersDir    = path.join(__dirname, 'templates/_helpers'),
    partialsDir   = path.join(__dirname, 'templates/_partials'),
...
function Mailman ( options ) {
  ...
  partials.forEach(filename => {
    let template = fs.readFileSync(path.join(partialsDir, filename), 'utf8'),
...
Mailman.prototype.send = function ( templateName, options ) {
  ...
  if ( process.env.ALLOW_EMAILS !== 'true' ) {
    winston.debug('Please set env var "allowEmails" to true to send emails.');
    return Promise.resolve();
  }

  winston.debug(chalk.dim('Mail :: Rendering content for email with template:', templateName));
...
Mailman.prototype.__getTemplate = function (name) {
  ...
  let template = new EmailTemplate(path.join(this.__templatesDir, name), {
