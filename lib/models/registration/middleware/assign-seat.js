...
let findBlockInArray = (a, time) => _.find(a, block => {
  console.log('checking block', block,  moment(time).hour() >= block[0] && moment(time).hour() < block[1], 'against time', moment(time).hour());
  return moment(time).hour() >= block[0] && moment(time).hour() < block[1]
});
...
function middleware (next) {
  ...
  let findBlock = time => {
    ...
    console.log('days', { days: { $in: [ moment(_time).day() ] } });
  ...
  let incrementBlock = function*(time) {
    ...
    console.log('found block increment on', block[1]);
  ...
  let findNextBlock = function*(start) {
    while ( yield inBlackoutDate(start.toDate()) ) {
      console.log('incrementing:', start.format('M/D h:mma'));
