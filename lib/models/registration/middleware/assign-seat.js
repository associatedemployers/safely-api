...
function middleware (next) {
  ...
  let inBlackoutDate = function*(date, classes) {
    ...
    // console.log('CHECKING BLACKOUT FOR DATE:', date, 'IN BLOCK:', block);
    ...
    return !block[1] || (yield BlackoutDate.findOne({
      ...
      .then(blackouts => {
        console.log('BLACKOUTS FOUND:', blackouts);
        ...
        //blackout.blocks is a 2D array
        //blockRange si a 1D array
        // console.log('EVALUATING exceptions1!!!!:', exceptions, _classes, blackouts.filter(blackout=> {
        //   console.log(_classes.filter(c => !_.includes(exceptions, c)).length > 0 && blackouts.filter(blackout=> blackout.blocks && _.includes(blackout.blocks[0], blockRange[0])).length > 0); 
        console.log(_classes.filter(c => !_.includes(exceptions, c)).length > 0 && blackouts.filter(blackout=> blackout.blocks && _.includes(blackout.blocks[0], blockRange[0])).length > 0 || blackouts.filter(blackout=> blackout.blocks && _.includes(blackout.blocks[0], blockRange[0])).length === 0); 
        ...
        if (explicitClasses.length > 0) {
          ...
          // console.log('EVALUATING BLACKOUTS AGAINST CLASSES:', blackouts);
  ...
  let findNextBlock = function*(start, classes) {
    ...
    console.log('Finding next block starting at:', yield inBlackout());

    while (yield inBlackout()) {
      if (blackout.classExceptions && blackout.classExceptions.length > 0 || blackout.classExplicit && blackout.classExplicit.length > 0 || blackout.hubClassExceptions && blackout.hubClassExceptions.length > 0) {
