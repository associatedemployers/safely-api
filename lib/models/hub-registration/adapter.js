exports.create = function*(request, transaction, hash) {
  ...
  let registrationHeader = {
    ...
    CompanyName:          reg.company ? reg.companyName : null
  };
  ...
  if (reg.company){
    this.record.company = reg.company?._id || reg.company?.id || reg.company;
  }
  if (reg.creator){
    this.record.creator = record.creator?._id || reg.company?.id || record.creator;
  }
...
exports.update = function*(request, transaction, hash) {
  ...
  const sqlIdentifiers = reg.participants.map(participant => participant.sqlIdentifier).filter(Boolean);
