exports.create = function*(request, transaction, hash) {
  ...
  let registrationHeader = {
    ...
    CompanyName:          reg.company ? reg.company.name : reg.companyName ? reg.companyName : null
  };
  ...
  if (reg.company){
    this.record.company = (reg.company || {})._id || reg.company;
  }
  if (reg.creator){
    this.record.creator = (reg.creator || {})._id || record.creator;
  }
