const { forOwn, get, set, assign, isArray, toString } = require('lodash'),
      co = require('co'),
...
SQLMongooseAdapter.prototype.serialize = function (record) {
  ...
  if (hashMap.onlyInclude) {
    console.log('only include', hashMap.onlyInclude);
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!process.env.SQL_DRYRUN) {
        try {
          ...
          winston.error('Could not save document:', e);
          continue;
        }
      } else {
        document = data;
...
SQLMongooseAdapter.prototype.getUpdates = function (schema) {
  ...
  return co(function*() {
    ...
    // Run discovery
    let request = new adapter.sql.Request(connection),
        q = config.updateDiscoveryQuery ||
          `select * from ${config.table} where SafelyID is not null and ModifiedDate is not null and Version is null`;

    request.verbose = true;
    let records = yield request.query(q);

    winston.debug(`SQL Document Updates :: Found ${records.length} to be imported.`);
    ...
    for (let i = 0; i < records.length; i++) {
      ...
      if (!process.env.SQL_DRYRUN) {
        try {
          document = yield currentRecord.save();
        } catch (e) {
