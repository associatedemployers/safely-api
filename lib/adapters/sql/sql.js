...
SQLMongooseAdapter.prototype.adapterOverrideFor = function (record, op) {
  try {
    const adapterTryPath = path.join(__dirname, `../../models/${this.modelNameFor(record).dasherized}/adapter`);
    var config = require(adapterTryPath)[op];
  } catch (e) {
    // We don't care.
    return false;
  }
...
SQLMongooseAdapter.prototype.importNewRecords = function (schema) {
  ...
  return co(function*() {
    ...
    winston.debug('SQL Document Recognizer :: Connecting to SQL.');
